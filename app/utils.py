...
class SentenceWithPlayback(gr.Interface):
    def __init__(self, fn, **kwargs):
        super().__init__(fn=fn, inputs=gr.inputs.Textbox(label="句子"),
                         outputs=gr.outputs.Audio(label="播放", autoplay=False), **kwargs)

    def run(self, inputs):
        # 调用父类的run方法来执行函数并获取输出
        outputs = super().run(inputs)
        # 将输出的音频文件路径更新到文本框中
        self.inputs[0].update_value(outputs)
        return outputs
